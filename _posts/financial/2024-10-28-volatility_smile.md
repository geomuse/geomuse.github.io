...
```py
# Bates 模型特征函数（包含跳跃项）
def bates_characteristic_function(u, params, S0, r, q, T):
...
# 期权价格积分（与 Heston 类似）
def bates_option_price(K, params, S0, r, q, T):
    # 类似于 Heston 模型的定价，但使用 Bates 的特征函数
    def integrand(u):
        cf = bates_characteristic_function(u - 1j * 0.5, params, S0, r, q, T)
...
def bates_calibration_error(params):
    kappa, theta, sigma, rho, v0, lambd, muJ, sigmaJ = params
    model_prices = [bates_option_price(K, params, S0, r, q, T) for K in market_strikes]
...
# 优化
result = minimize(bates_calibration_error, initial_guess, bounds=bounds, method='L-BFGS-B')
